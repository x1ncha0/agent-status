...
const root = process.env.AGENT_STATUS_DATA_DIR || path.join(process.env.LOCALAPPDATA, 'AgentStatus');
const seen = new Set();
const events = [];
const watchers = [];
for (const agent of ['claude', 'codex']) {
  ...
  watchers.push(fs.watch(directory, (_kind, name) => {
    ...
    } catch { /* Main có thể đã đọc/xóa file; observer không can thiệp. */ }
  }));
}

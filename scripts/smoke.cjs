...
async function sendEvent(agent, hook, session = 'smoke-session') {
  ...
  await fs.writeFile(
    `${file}.tmp`,
    JSON.stringify({ agent, session_id: session, hook_event_name: hook, timestamp: Date.now() }),
  );
  await fs.rename(`${file}.tmp`, file);
}
const visibleAgents = (win) =>
  win.webContents.executeJavaScript(
    "[...document.querySelectorAll('.agent')].filter(element => element.getBoundingClientRect().width > 0).map(element => element.id)",
  );
app.whenReady().then(async () => {
  try {
    ...
    await until(async () => {
      try {
        return Boolean(
          JSON.parse(await fs.readFile(path.join(root, 'setup.json'), 'utf8')).deferred,
        );
      } catch {
        return false;
      }
    ...
    report.checks.push(
      'First launch offers setup; defer writes no CLI settings; manual setup installs to isolated homes; subsequent launch is silent; Codex trust/new-session guidance present',
    );
    ...
    assert.equal(
      win.isVisible(),
      false,
      'Reopening the app with no active agents does not show an empty window',
    );
    ...
    const centered = await win.webContents.executeJavaScript(
      '({ center: document.querySelector("#claude .dot").getBoundingClientRect().x + document.querySelector("#claude .dot").getBoundingClientRect().width / 2, viewport: innerWidth })',
    );
    ...
    assert.equal(
      await win.webContents.executeJavaScript('typeof window.agentStatus.get'),
      'function',
    );
    assert.equal(await win.webContents.executeJavaScript('typeof window.require'), 'undefined');
    assert.equal(
      await win.webContents.executeJavaScript(
        "getComputedStyle(document.querySelector('.status-content')).getPropertyValue('-webkit-app-region')",
      ),
      'drag',
    );
    assert.equal(
      await win.webContents.executeJavaScript(
        "getComputedStyle(document.querySelector('main')).getPropertyValue('-webkit-app-region')",
      ),
      'no-drag',
    );
    report.checks.push(
      'Sandbox/preload IPC present; renderer has no require; CSS drag region present',
    );
    ...
    win.once('always-on-top-changed', (_event, value) => {
      topmostEvent = value;
    });
    showWindow(win);
    ...
    const resizeChecks = await new Promise((resolve, reject) =>
      execFile(
        'powershell.exe',
        [
          '-NoProfile',
          '-NonInteractive',
          '-File',
          path.resolve('scripts/Test-WindowResize.ps1'),
          '-Handle',
          nativeHandle,
        ],
        { windowsHide: true, timeout: 20000 },
        (error, stdout, stderr) =>
          error ? reject(new Error(stderr || error.message)) : resolve(JSON.parse(stdout)),
      ),
    );
    ...
    const scaled = await win.webContents.executeJavaScript(
      '({ scale: parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--ui-scale")), font: parseFloat(getComputedStyle(document.body).fontSize), dot: document.querySelector(".dot").getBoundingClientRect().width })',
    );
    ...
    restored.destroy();
    report.checks.push(
      'Real mouse resizing: right, bottom, NW corner, SE shrinking; layout fills window; position and size persist and restore',
    );
    win.setSize(90, 45);
    await sleep(150);
    assert.deepEqual(win.getSize(), [90, 45]);
    await fs.writeFile(
      path.join(root, 'minimum-size.png'),
    ...
    for (const agent of ['claude', 'codex']) {
      for (const [hook, status] of [
        ...
        await until(() =>
          win.webContents.executeJavaScript(
            `document.querySelector('#${agent} .dot').className === 'dot ${status}'`,
          ),
        );
        assert.ok(
          (await visibleAgents(win)).includes(agent),
          `${agent} remains visible when ${status}`,
        );
    ...
    for (const [hook, tool, id, status] of [
      ...
      await fs.writeFile(
        path.join(root, 'events/codex', `${require('node:crypto').randomUUID()}.json`),
        JSON.stringify({
          agent: 'codex',
      ...
      await until(() =>
        win.webContents.executeJavaScript(
          `document.querySelector('#codex .dot').className === 'dot ${status}'`,
        ),
      );
      if (status === 'stuck') {
        assert.ok(
          (
            await win.webContents.executeJavaScript(
              "document.getElementById('codex').getAttribute('aria-label')",
            )
          ).includes('Cần bạn can thiệp'),
        );
      }
    ...
    report.checks.push(
      'Green=ready, yellow=thinking/working, red=needs user; Vietnamese accessible label',
    );
    await fs.writeFile(
    ...
    await until(
      async () =>
        (await win.webContents.executeJavaScript('window.agentStatus.get()')).find(
          (state) => state.agent === 'claude',
        ).reason === 'Session đã bắt đầu',
    );
    assert.deepEqual(
      await visibleAgents(win),
      ['claude', 'codex'],
      'A second active session keeps its agent visible',
    );
    await sendEvent('claude', 'SessionEnd', 'another-session');
    await until(async () => (await visibleAgents(win)).join() === 'codex');
    assert.equal(win.isVisible(), true);
    await fs.writeFile(
      path.join(root, 'single-agent.png'),
      (await win.webContents.capturePage()).toPNG(),
    );
    ...
    await until(() =>
      win.webContents.executeJavaScript(
        "document.querySelector('#codex .dot').className === 'dot stuck'",
      ),
    );
    await sleep(650);

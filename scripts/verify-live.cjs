...
app.whenReady().then(async () => {
  try {
    ...
    const dot = await win.webContents.executeJavaScript("document.querySelector('#codex .dot').className");
    ...
    await fs.writeFile(path.join(directory, 'report.json'), JSON.stringify({ state, dot }, null, 2));
    console.log(JSON.stringify({ state, dot, result: 'Live Codex hook -> monitor -> IPC -> yellow DOM' }, null, 2));
    app.exit(0);

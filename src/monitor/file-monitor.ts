...
export function parseEvent(value: unknown): HookEvent | undefined {
  ...
  const result: HookEvent = { agent: e.agent, session_id: e.session_id, hook_event_name: e.hook_event_name, timestamp: e.timestamp };
  ...
  if (e.owner_pid !== undefined || e.owner_started_at !== undefined) {
    if (!Number.isSafeInteger(e.owner_pid) || Number(e.owner_pid) <= 0 ||
        !Number.isSafeInteger(e.owner_started_at) || Number(e.owner_started_at) <= 0) return;
    result.owner_pid = Number(e.owner_pid); result.owner_started_at = Number(e.owner_started_at);
  }
  return result;
}
...
export class FileMonitor implements Monitor {
  ...
  private saved = '';
  constructor(private directory: string, agent: Agent, classify: Classifier, private ownerAlive: OwnerAlive = createOwnerProbe()) {
    ...
    try {
      ...
      if (!this.restored) {
        try {
          ...
          if (Array.isArray(cached)) for (const value of cached) {
      ...
      for (const event of [...this.store.events(), ...events]) if (event.owner_pid) {
        const key = `${event.owner_pid}:${event.owner_started_at}`;
        if (!owners.has(key)) owners.set(key, this.ownerAlive(event.owner_pid, event.owner_started_at!));
      }
      const alive = new Map(await Promise.all([...owners].map(async ([key, result]) => [key, await result] as const)));
      const active = (event: HookEvent) => !event.owner_pid || alive.get(`${event.owner_pid}:${event.owner_started_at}`);
      for (const event of this.store.events()) if (!active(event)) this.store.forget(event.session_id);
      events.sort((a, b) => a.timestamp - b.timestamp).filter(active).forEach(event => this.store.accept(event));
      const serialized = JSON.stringify(this.store.events().filter(event => event.owner_pid && event.hook_event_name !== 'SessionEnd'));
      ...
      this.restored = true;
      for (const file of consumed) await unlink(file).catch(error => { if (error.code !== 'ENOENT') throw error; });
      this.error = '';

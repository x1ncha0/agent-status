...
interface SetupPaths { dataDir: string; integrationDir: string; claudeHome: string; codexHome: string }
const execute = promisify(execFile);
const nextSteps = 'Claude Code: mở lại CLI rồi gửi một yêu cầu.\n\n' +
...
export function createIntegrationSetup(win: BrowserWindow, paths: SetupPaths) {
  ...
  const run = async (mode: '-Check' | '-Apply') => {
    ...
    return execute(shell, ['-NoProfile', '-NonInteractive', '-File', installer, mode,
      '-DataDir', paths.dataDir, '-ClaudeHome', paths.claudeHome, '-CodexHome', paths.codexHome
    ], { windowsHide: true, timeout: 20000, maxBuffer: 1024 * 1024 });
  };
  const check = async () => {
    const { stdout } = await run('-Check');
    installation = JSON.parse(stdout) as Installation;
    return installation;
  };
  const show = async (automatic = false) => {
    ...
    try {
      ...
      const fingerprint = createHash('sha256').update(await readFile(installer))
        .update(await readFile(path.join(paths.integrationDir, 'Write-AgentEvent.ps1'))).digest('hex');
      if (automatic) {
        try { if (JSON.parse(await readFile(preferenceFile, 'utf8')).deferred === fingerprint) return; }
        catch { /* No deferred setup on first launch. */ }
      }
      if (win.isDestroyed()) return;
      const summary = current.agents.map(agent => `${agent.agent === 'claude' ? 'Claude Code' : 'Codex'}: ${agent.configured && current.writerCurrent ? 'đã cài kết nối' : 'cần thiết lập / cập nhật'}`).join('\n');
      const { response } = await dialog.showMessageBox(win, {
        type: 'info', title: 'Agent Status — Thiết lập kết nối',
        message: current.needsInstall ? 'Kết nối với Claude Code và Codex' : 'Kết nối đã được thiết lập',
        ...
        defaultId: 0, cancelId: current.needsInstall ? 1 : 0
      });
      ...
      await dialog.showMessageBox(win, {
        type: 'info', title: 'Agent Status', message: 'Đã cài kết nối',
      ...
      if (!win.isDestroyed()) await dialog.showMessageBox(win, {
        type: 'error', title: 'Agent Status', message: 'Chưa thể thiết lập kết nối',
        detail: 'Mở menu khay hệ thống → Thiết lập kết nối để thử lại.\n\n' + (error as Error).message,
        buttons: ['Đóng']
      });
    } finally { busy = false; }
  };
  return {
    ...
    hint: (agent: Agent) => {
      if (!installation?.writerCurrent || !installation.agents.find(item => item.agent === agent)?.configured)
        return 'Nhấp phải biểu tượng khay hệ thống → Thiết lập kết nối.';
      return agent === 'codex'
        ? 'Mở Codex trong terminal mới, vào /hooks để Review / Trust rồi gửi một yêu cầu.'
        : 'Mở lại Claude Code rồi gửi một yêu cầu.';
    }
  };
}
